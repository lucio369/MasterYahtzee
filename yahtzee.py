...
class Player:
    def __init__(self):  # init function for Player
...
def runYahtz(ctx):
    ...
    check if author is still involved (whiteItem)
    run game for author
    remove finished authors and add score to leaderboard
    if no authors remaining, reveal leaderboard
    '''

    if bot.game(bot.playerDict[ctx.author],ctx.content):
        out = ctx.author + ':' + bot.playerDict[ctx.author].scoreCard['"Total Score']
        bot.leaderboard.append(out)
        del bot.playerDict[ctx.author]
        if not bot.playerDict:
            bot.gaming = False
...
async def on_message(ctx):
    ...
    if bot.gaming:
        if ctx.author in bot.playerDict:
            await bot.runYahtz()
